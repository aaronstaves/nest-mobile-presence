var NestApi = function() {
  ...
  function _checkPresence( callback ) {
    ...
    rp.get(opt)
      .then( function(ret) {
        ...
        callback({ presence: presence, structure: structure, cameras: ret.devices.cameras });
      })
      .catch( function (err) {
        var ret = null;
  ...
  function _setCameras( is_streaming, cameras, callback ) {

		Object.keys( cameras ).forEach( function(camera) {
			var opt = {
				uri: base_url + 'devices/cameras/' + camera,
				json: true,
				followAllRedirects: true,
				body: {
			...
			rp.put(opt)
				.then( function(ret) {
					callback ( is_streaming, cameras[camera] );
				})
				.catch( function (err) {
					var ret = null;
					if ( err.response && err.response.body ) {
